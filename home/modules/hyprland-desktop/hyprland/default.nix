    ...
    settings = {
      ...
      bind =
        [
          ...
          "${mod} CTRL,S,exec,${pkgs.hyprshot}/bin/hyprshot -m region -zrs | ${pkgs.satty}/bin/satty -f - -o \"$HOME/Pictures/screenshots/screenshot-$(date +'%Y-%m-%d_%H-%M-%S').png\" --early-exit --actions-on-enter save-to-clipboard --copy-command 'wl-copy' --initial-tool brush --no-window-decoration"
